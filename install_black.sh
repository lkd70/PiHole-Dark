...
while true; do
    read -p "Install LKD70's and xMate420x's Black Theme now? (Y/n): " yn
    case $yn in
        [Yy]* ) 
	    git clone https://github.com/lkd70/PiHole-Dark.git temp
            rm -f admin/style/vendor/AdminLTE.min.css
            cp temp/Black/admin/style/vendor/AdminLTE.min.css admin/style/vendor/AdminLTE.min.css
            cp temp/Black/admin/style/vendor/AdminLTE.css admin/style/vendor/AdminLTE.css
	    rm -rf temp/
			break;;
        [Nn]* ) break;;
        * ) echo "Please answer Yes (y) or No (n).";;
